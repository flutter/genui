...
-   If you were using `genui_a2a`, it is still supported but has been updated to use the new `A2uiAgentConnector` pattern, and no longer has a `ContentGenerator`.
...
If you are using `genui_a2a` (A2A/A2UI adapter) to connect to a remote A2A/A2UI agent:
