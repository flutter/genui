...
Map<String, Object?> _reconstructBindings(
...
class ManageUiTool {
        ...
        invokeFunction: (args) async {
          ...
          final nodes = (layoutMap['nodes'] as List)
            ...
            final properties = (node['properties'] as List?)
                ?.cast<Map<String, Object?>>() ??
                [];
            final bindings = (node['bindings'] as List?)
                ?.cast<Map<String, Object?>>() ??
                [];
            return {
              'id': node['id'],
  ...
  AiTool<Map<String, Object?>> get patchLayout => DynamicAiTool(
        name: 'patchLayout',
        description:
            'Applies a set of patch operations to the layout of a'
            ' specific surface.',
        parameters: Schema.object(
          properties: {
            'surfaceId': Schema.string(
            ...
            'operations': Schema.list(
              description:
                  'An array of FCP layout patch operations. See the '
                  'FCP documentation for the full specification of the patch '
                  'format.',
              items: fcpLayoutOperationSchema,
            ),
          },
