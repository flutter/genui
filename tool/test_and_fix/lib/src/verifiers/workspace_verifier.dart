...
class WorkspaceVerifier {
  ...
  Future<bool> verify({
        ...
        if (yaml is YamlMap) {
          ...
          if (name != null && name.isNotEmpty && versionStr != null) {
            try {
              ...
              packagePaths[name] = fs.path.relative(project.path, from: repoRoot.path);
            } on FormatException {
              _log.warning('Warning: Could not parse version "$versionStr" for package $name.');
            }
          }
        }
      } catch (e) {
    ...
    for (final MapEntry<String, YamlMap> entry in packagePubspecs.entries) {
        ...
        if (deps is YamlMap) {
          for (final depEntry in deps.entries) {
            ...
            if (localPackages.containsKey(depName)) {
              ...
              } else if (constraintObj is YamlMap && constraintObj.containsKey('version')) {
              ...
              try {
                final VersionConstraint constraint = VersionConstraint.parse(constraintStr);
                ...
                if (!constraint.allows(actualVersion)) {
                 ...
                 _log.warning('Warning: Could not parse constraint "$constraintStr" for dependency $depName in $consumerName.');
              }
            }
          }
        }
      ...
      if (passedForPackage) {
        _log.info('✅ $consumerPath: all internal workspace constraints satisfied.');
      }
    }

    if (!allPassed) {
      _log.severe('\n❌ Workspace version constraint verification failed.');
      _log.severe('Dart workspace resolution will silently fall back to pub.dev if local constraints are not met.');
      _log.severe('Please update the failing constraints to allow the local sibling version.');
      return false;
    }
