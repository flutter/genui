...
final class ChatSession extends ChatBackend {
  ...
  void _init(Catalog catalog) {
    ...
    // Reflect UI interactions (e.g. button taps generated by the LLM) as
    // visible user-message bubbles, and reset the current AI bubble so the
    // LLM's reply lands in a fresh one. The actual forwarding to the LLM is
    // already wired up by Conversation listening on controller.onSubmit.
    _surfaceController.onSubmit.listen((message) {
      for (final part in message.parts) {
        if (!part.isUiInteractionPart) continue;
        final decoded = jsonDecode(part.asUiInteractionPart!.interaction);
        ...
        _messages.add(Message(isUser: true, text: 'You clicked: $name'));
        notifyListeners();
      }
    });
